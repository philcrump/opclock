...
static void touch_readEvents(int touch_fd, void (*touch_callback)(int type, int x, int y), bool *app_exit_ptr)
{
  size_t i, rb;
...
static void touch_run(char *touch_path, void (*touch_callback)(int type, int x, int y), bool *app_exit_ptr)
{
  int inotify_fd, r;
  int touch_fd;
  ...
  while (!*app_exit_ptr)
  {
    ...
    else if(FD_ISSET(inotify_fd, &inotify_fdset))
    {
      ...
      for (p = buf; p < buf + pending_length; )
      {
        ...
        touch_readEvents(touch_fd, touch_callback, app_exit_ptr);
...
void *touch_thread(void *arg)
{
  ...
  touch_run(touchscreen_path, &touch_process, app_exit_ptr);
