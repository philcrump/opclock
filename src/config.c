...
int config_handler(void* user, const char* section, const char* name, const char* value)
{
  ...
  else if (MATCH("events", "source_type"))
  {
    if(strcmp("FILE", value) == 0)
    {
      app_config_ptr->event_source = EVENT_SOURCE_FILE;
    }
    else if(strcmp("HTTP", value) == 0)
    {
      app_config_ptr->event_source = EVENT_SOURCE_HTTP;
    }
    else // if(strcmp("DEMO", value) == 0)
    {
      app_config_ptr->event_source = EVENT_SOURCE_DEMO;
    }
  }
